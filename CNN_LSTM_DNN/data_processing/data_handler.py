...
def load_abbreviation(path='/resource/abbreviations.txt'):
    path = os.getcwd()[:os.getcwd().rfind('\\')] + path
    print("..............................................")
    print(path)
...
def parsedata(lines, word_list, split_word_list, emoji_dict, abbreviation_dict, normalize_text=False,
    ...
    for i, line in lines.iterrows():
        ...
        try:
            ...
            # tweet text
            target_text = TweetTokenizer().tokenize(line['response'].strip())
            if (at_character):
                target_text = [c for c in line['response'].strip()]
            ...
            if line['context']:
                C = '' 
                for c in line['context']:
                    C = C + c.strip()
                context = TweetTokenizer().tokenize(C)#[-1].strip())
...
def loaddata(filename, word_file_path, split_word_path, emoji_file_path, normalize_text=False, split_hashtag=False,
    ...
    lines = pd.read_json(filename, orient = 'columns', lines = True)
    #replace label with 0 & 1
    try:
        lines['label'].replace('SARCASM', 1, inplace=True)
        lines['label'].replace('NOT_SARCASM', 0, inplace = True)
    except:
        None
