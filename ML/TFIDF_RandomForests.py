...
data_raw = pd.read_json(basepath + '/data/train.jsonl', orient = 'columns', lines = True)
test_raw = pd.read_json(basepath + '/data/test.jsonl', orient = 'columns', lines = True)
...
#%%
#train and test
pipeline = pipeline_builder(vectorizer = tfidf, classifier = svm)
test_accuracy, train_accuracy, report = accuracy_summary(pipeline, X_train, y_train, X_eval, y_eval)
#uncomment for generating output file
#generate_output(pipeline, X, y, test_X, test_ID, basepath)
